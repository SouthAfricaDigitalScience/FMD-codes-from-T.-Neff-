...
int main(int argc, char *argv[])
{
  ...
  int constdequadrupole=0, constdeqval=0;
  ...
  while((c = getopt(argc, argv, "j:K:p:ol:m:s:T:L:S:J:MR:D:Q:O:B:G:")) != -1)
    switch (c) {
    case 'j':
      j = atoi(optarg);
      break;
...
/*   } */
  ...
  // minimize !
  MinimizeDONLP2proj(&Int, j, k, par, &AngPar, Const, nconst, &P, &q, maxsteps, 
		     log, parafile); 
  ...
  fprintf(outfp, "\n# projected on J^pi = %s, K = %s\n", 
	  AngmomtoStr(j, par == +1 ? 0 : 1), AngmomtoStr(k, par == +1 ? 0 : 1));
