/**
  ...
  \file calcdensitiespnp.c

  calc parity projected density cuts in coordinate space 
...
int main(int argc, char* argv[])
{
  ...
  char c;
  while ((c = getopt(argc, argv, "c:p:nl:ogC:Pqv:")) != -1)
    switch (c) {
    case 'g':
      debug=1;
      break;
  ...
  snprintf(datafile, 255, "%s.denspnp", slaterdetfile);;
  ...
  for (v=0; v<3; v++)
    if (view[v]) {
      ...
      calcDensitiesCoordinateParity(&Q, par, v, npoints, xmax, dens);
  ...
  snprintf(scriptfile, 255, "%s.denspnp.script", slaterdetfile);
  ...
  if (portrait)
    fprintf(scriptfp, 
	  "pos = initpost('%s.denspnp.eps',%d,%d, gapx=1.5, gapy=-0.01, /color)\n",
	  slaterdetfile, nplots[1], nplots[0]);
  else
    fprintf(scriptfp, 
	  "pos = initpost('%s.denspnp.eps',%d,%d, gapx=-0.01, gapy=1.5, /color)\n",
  ...
  char epsfile[255];
  snprintf(epsfile, 255, "%s.denspnp.eps", slaterdetfile);
