...
Parameterization ParameterizationAMD = {
  name : "AMD",
  Pararead : AMDread,
  Parawrite : AMDwrite,
  Paraclone : AMDclone,
  ParainitSlaterDet : AMDinitSlaterDet,
  ParatoSlaterDet : AMDtoSlaterDet,
  ParaprojectgradSlaterDet : AMDprojectgradSlaterDet
};
...
void AMDclone(const Para* q, Para* qp)
{
  ...
  qp->ngauss = q->ngauss;
  qp->internals = malloc(sizeof(AMDInternals));
  AMDInternals *qpint = qp->internals;
  AMDInternals *qint = q->internals;
...
int AMDread(FILE* fp, Para* q)
{
  ...
  qint = q->internals = (AMDInternals*) malloc(sizeof(AMDInternals));
  ...
  if (strcmp(Pname, "AMD")) {
    fprintf(stderr, "not a AMD parameter set\n");
    return -1;
  }
