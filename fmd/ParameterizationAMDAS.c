...
Parameterization ParameterizationAMDAS = {
  name : "AMDAS",
  Pararead : AMDASread,
  Parawrite : AMDASwrite,
  Paraclone : AMDASclone,
  ParainitSlaterDet : AMDASinitSlaterDet,
  ParatoSlaterDet : AMDAStoSlaterDet,
  ParaprojectgradSlaterDet : AMDASprojectgradSlaterDet
};
...
void AMDASclone(const Para* q, Para* qp)
{
  ...
  qp->ngauss = q->ngauss;
  qp->internals = malloc(sizeof(AMDASInternals));
  AMDASInternals *qpint = qp->internals;
  AMDASInternals *qint = q->internals;
...
int AMDASread(FILE* fp, Para* q)
{
  ...
  qint = q->internals = (AMDASInternals*) malloc(sizeof(AMDASInternals));
  ...
  if (strcmp(Pname, "AMDAS")) {
    fprintf(stderr, "not a AMDAS parameter set\n");
    return -1;
  }
