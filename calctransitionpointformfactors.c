/**
  ...
  \file calctransitionpointformfactors.c

  calculate transition point formfactors
...
int main(int argc, char* argv[])
{
  ...
  snprintf(outfile, 255, "%s-%s.%d-%s.%d.transpointformfactorsred", 
  ...
  for (i=0; i<NMULTIPOLES; i++)
    if (multi[i]) {
      writeTransitionPointFormfactors(datafp, &P, &FfP,
				      i,
				      jfin, pfin, beta, jini, pini, alpha, 
				      ffactortrans[l], &E);
    }
