/**
  ...
  \file calctransitionchargeformfactors.c

  calculate transition charge formfactors
...
int main(int argc, char* argv[])
{
  ...
  int jfin=-1, jini=-1, pini=-1, pfin=-1, alpha=0, beta=0;
  ...
  fprintf(stderr, "Jini: %d%c, Jfin: %d%c, alpha: %d, beta: %d\n", 
  ...
  // output of formfactors

  char outfile[255];

  snprintf(outfile, 255, "%s-%s.%d-%s.%d.transchargeformfactorsred", 
  ...
  for (i=0; i<NMULTIPOLES; i++)
    if (multi[i]) {
      writeTransitionChargeFormfactors(datafp, &P, &FfP,
				       i,
				       jfin, pfin, beta, jini, pini, alpha, 
				       ffactortrans[l], &E);
    }
