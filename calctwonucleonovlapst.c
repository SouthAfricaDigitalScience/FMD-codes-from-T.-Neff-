/**
  ...
  \file calctwonucleonovlapst.c

  calculate two-nucleon ovlaps in T-coordinates
...
int main(int argc, char* argv[])
{
  ...
  initOpTwoNucleonOvlapsT(&SAP);

  void* specamplitudeme[NSPECT][nA*nB];
  ...
  for (b=0; b<nB; b++)
	...
	specamplitudeme[i][a+b*nA] = 
	  initprojectedMBME(&P, &OpTwoNucleonOvlapT[i]);

      havetocalc = 0;
      for (i=0; i<NSPECT; i++)
	if (multi[i])
	  if (readprojectedMBMEfromFile(mbAfile[a], mbBfile[b], &P,
					&OpTwoNucleonOvlapT[i], 
					SA[a], SB[b],
					specamplitudeme[i][a+b*nA]))
	    havetocalc = 1;
...
#ifdef MPI
	calcprojectedMBMEsmpi(&P, &OpTwoNucleonOvlapsT, &QA[a], &QB[b],
			      SA[a], SB[b], me);
#else
	calcprojectedMBMEs(&P, &OpTwoNucleonOvlapsT, &QA[a], &QB[b],
			   SA[a], SB[b], me);
#endif
	for (i=0; i<NSPECT; i++)
	  writeprojectedMBMEtoFile(mbAfile[a], mbBfile[b], &P, 
				   &OpTwoNucleonOvlapT[i], SA[a], SB[b], 
				   specamplitudeme[i][a+b*nA]);
      }
    }


  void* specamplitude[NSPECT];

  for (i=0; i<NSPECT; i++)   
    if (multi[i]) {
      specamplitude[i] = 
	initprojectedtransitionVector(&P, &OpTwoNucleonOvlapT[i], nA, nB);
      calctransitionprojectedMBME(&P, &OpTwoNucleonOvlapT[i], 
				  specamplitudeme[i], 
  ...
  snprintf(outfile, 255, "%s-%s.%d-%s-%s.%d.twonucleonovlapst", 
  ...
  char* speclabel[NSPECT] = { "0Ss00", "0Pp11", "0Dd02", "0Ff13", "0Gg04" };
