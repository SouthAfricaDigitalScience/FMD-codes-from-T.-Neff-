/**
  ...
  \file MinimizerDONLP2multivappiso.c
...
// K-mixing threshold
#define THRESHKMIX 0.1
#define MINNORMKMIX 0.01

// Multiconfig mixing threshold
#define THRESHMULTI 0.0000001
...
static void mirrorSlaterDet(SlaterDet* Q)
{
  int i;
  int Z=Q->Z; int N=Q->N;
                   ...
                   double* egood, complex double* Vgood, int* ngood)
{
  ...
  for (i=0; i<d; i++) {
    merits[i].idx = i;
    if (creal(norm[i]) < MINNORMKMIX*maxnorm)
                                    ...
                                    complex double* H, complex double* N, complex double* J2)
{
  ...
  for (i=0; i<nang; i++) {
	...
	for (kp=-j; kp<=j; kp=kp+2)
	  for (k=-j; k<=j; k=k+2) {
	    H[idxjmk(j,k,kp)] += w/4*(ip ? par : 1)*(iiso ? iso : 1)*
	      (j+1)/(8*M_PI2)*Djmkstar(j,k,kp,alpha,beta,gamma)*hme;
	    N[idxjmk(j,k,kp)] += w/4*(ip ? par : 1)*(iiso ? iso : 1)*
	      (j+1)/(8*M_PI2)*Djmkstar(j,k,kp,alpha,beta,gamma)*nme;
	    J2[idxjmk(j,k,kp)] += w/4*(ip ? par : 1)*(iiso ? iso : 1)*
	      (j+1)/(8*M_PI2)*Djmkstar(j,k,kp,alpha,beta,gamma)*j2me;
	  }
      }
    }
  }
...
void calcprojectedHamiltonianmatrix(const SlaterDet* Q, const SlaterDet* Qp,
                                    ...
                                    complex double* H, complex double* N, complex double* J2)
{
  // isospin projection not added yet !
      ...
      if (next<todo && running < mpisize-1) {	// supply slaves with work
        ...
        for (kp=-j; kp<=j; kp=kp+2)
          for (k=-j; k<=j; k=k+2) {
            ...
            H[idxjmk(j,k,kp)] += w/4*(hme[0] + par* hme[1]);
            N[idxjmk(j,k,kp)] += w/4*(nme[0] + par* nme[1]);
            J2[idxjmk(j,k,kp)] += w/4*(j2me[0] + par* j2me[1]);
          }
  ...
  // contribution from mirror Slaterdet
  {
    int task = TASKHAMILTONIANOD;
    BroadcastTask(&task);
      ...
      if (next<todo && running < mpisize-1) {	// supply slaves with work
        ...
        for (kp=-j; kp<=j; kp=kp+2)
          for (k=-j; k<=j; k=k+2) {
            ...
            H[idxjmk(j,k,kp)] += w/4*iso*(hme[0] + par* hme[1]);
            N[idxjmk(j,k,kp)] += w/4*iso*(nme[0] + par* nme[1]);
            J2[idxjmk(j,k,kp)] += w/4*iso*(j2me[0] + par* j2me[1]);
          }
  ...
  for (i=0; i<nang; i++) {
	...
	for (kp=-j; kp<=j; kp=kp+2)
	  for (k=-j; k<=j; k=k+2) {
	    ...
	    H[idxjmk(j,k,kp)] += w/4*(ip ? par : 1)*(iiso ? iso : 1)*
	      (j+1)/(8*M_PI2)*Djmkstar(j,k,kp,alpha,beta,gamma)*dh->val;
	    N[idxjmk(j,k,kp)] += w/4*(ip ? par : 1)*(iiso ? iso : 1)*
	      (j+1)/(8*M_PI2)*Djmkstar(j,k,kp,alpha,beta,gamma)*dn->val;

	    addmulttogradSlaterDet(&dH[idxjmk(j,k,kp)], dh, 
				   w/4*(ip ? par : +1)*(iiso ? iso : 1)*
				   (j+1)/(8*M_PI2)*Djmkstar(j,k,kp,alpha,beta,gamma));
	    addmulttogradSlaterDet(&dN[idxjmk(j,k,kp)], dn, 
				   w/4*(ip ? par : +1)*(iiso ? iso : 1)*
				   (j+1)/(8*M_PI2)*Djmkstar(j,k,kp,alpha,beta,gamma));
	  }
      }
                                        ...
                                        gradSlaterDet* dH, gradSlaterDet* dN)
{
      ...
      if (next<todo && running < mpisize-1) {	// supply slaves with work
        ...
        for (kp=-j; kp<=j; kp=kp+2)
          for (k=-j; k<=j; k=k+2) {
            ...
            H[idxjmk(j,k,kp)] += w/4* dh->val;
            addmulttogradSlaterDet(&dH[idxjmk(j,k,kp)], dh, w/4);
          }    
        ...
        for (kp=-j; kp<=j; kp=kp+2)
          for (k=-j; k<=j; k=k+2) {
            ...
            H[idxjmk(j,k,kp)] += w/4*par* dh->val;
            addmulttogradSlaterDet(&dH[idxjmk(j,k,kp)], dh, w/4*par);
          }    
        ...
        for (kp=-j; kp<=j; kp=kp+2)
          for (k=-j; k<=j; k=k+2) {
            ...
            N[idxjmk(j,k,kp)] += w/4* dn->val;
            addmulttogradSlaterDet(&dN[idxjmk(j,k,kp)], dn, w/4);
          }        
        ...
        for (kp=-j; kp<=j; kp=kp+2)
          for (k=-j; k<=j; k=k+2) {
            ...
            N[idxjmk(j,k,kp)] += w/4*par* dn->val;
            addmulttogradSlaterDet(&dN[idxjmk(j,k,kp)], dn, w/4*par);
          }        
  ...
  // contribution from mirrored Slaterdet
  {
    int task = TASKGRADHAMILTONIANOD;
    BroadcastTask(&task);
      ...
      if (next<todo && running < mpisize-1) {	// supply slaves with work
        ...
        for (kp=-j; kp<=j; kp=kp+2)
          for (k=-j; k<=j; k=k+2) {
            ...
            H[idxjmk(j,k,kp)] += w/4*iso* dh->val;
            addmulttogradSlaterDet(&dH[idxjmk(j,k,kp)], dh, w/4*iso);
          }    
        ...
        for (kp=-j; kp<=j; kp=kp+2)
          for (k=-j; k<=j; k=k+2) {
            ...
            H[idxjmk(j,k,kp)] += w/4*iso*par* dh->val;
            addmulttogradSlaterDet(&dH[idxjmk(j,k,kp)], dh, w/4*iso*par);
          }    
        ...
        for (kp=-j; kp<=j; kp=kp+2)
          for (k=-j; k<=j; k=k+2) {
            ...
            N[idxjmk(j,k,kp)] += w/4*iso* dn->val;
            addmulttogradSlaterDet(&dN[idxjmk(j,k,kp)], dn, w/4*iso);
          }        
        ...
        for (kp=-j; kp<=j; kp=kp+2)
          for (k=-j; k<=j; k=k+2) {
            ...
            N[idxjmk(j,k,kp)] += w/4*iso*par* dn->val;
            addmulttogradSlaterDet(&dN[idxjmk(j,k,kp)], dn, w/4*iso*par);
          }        
                              ...
                              double* eintr, double* eproj)
{
  ...
  calcprojectedHamiltonianmatrix(Qp, Qp, Int, j, par, iso, angpara,
                                 H, N, J2);

  // K-mixing for Qp
  double ed[j+1];

  kmixingstates(H, N, j, THRESHKMIX, ed, V, &dim);
				  ...
				  double* eintr, double* eproj)
{
  ...
  *eintr= creal(hintr/nintr);
  ...
  // projected matrix elements for Qp
  calcgradprojectedHamiltonianmatrix(Qp, Qp, Int, j, par, iso, angpara,
                                     H, N,
                                     dH, dN);
  ...
  kmixingstates(H, N, j, THRESHKMIX,
                ed, V, &dim);
...
void FORTRAN(ef)(const double* x, double* fx)
{
  ...
  calcprojectedHamiltonian(Min.Q,
			   Min.Int,
			   Min.j, Min.par, Min.iso, Min.ival,
                           Min.projpar,
                           &eintr, &eproj);
