...
int main(int argc, char* argv[])
{
  ...
  while ((c = getopt(argc, argv, "fj:p:a:l:rq:n:P:")) != -1)
    switch (c) {
    case 'f':
      fb = 1;
      break;
  ...
  snprintf(outfile, 255, "%s-%s.%d.chargedens", 
  ...
  for (i=0; i<NMULTIPOLES; i++)
    if (multi[i]) {
      writeChargeDensities(datafp, &P, &FfP, 
			   rmax, npointsr, 
			   Q[0].Z, i,
  ...
  if (fb) {
    ...
    for (i=0; i<NMULTIPOLES; i++)
      if (multi[i]) {
	writeChargeFourierBessel(datafp, &P, &FfP, 
				 rmax, npointsr, 
				 Q[0].Z, i,
				 j, pi, alpha, ffactorexp[i], &E);
      }
