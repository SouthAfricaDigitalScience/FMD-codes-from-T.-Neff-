/**
  ...
  \file minenergyconmultivappcm.c

  minimize energy in multiconfiguration calculation, only the parameters
  of last intrinsic state are varied
  Variation after angular-momentum and cm-projection
...
int main(int argc, char *argv[])
{
  ...
  while((c = getopt(argc, argv, "i:j:p:t:n:a:Lol:m:s:T:L:S:J:N:R:D:Q:O:P:CV:3")) != -1)
    switch (c) {
    case 'i':
      ival = atoi(optarg)-1;
      break;
...
/*   } */
  ...
  // set up angular momentum projection
  angintegrationpara angpara;
  initAngintegration(&angpara, projpar);

  // set up cm projection
  cmintegrationpara cmpara;
  initCMintegration(&cmpara, projpar);
  ...
  initWork(nfix, Qfix, j, par, &angpara, &cmpara, &Int, &Q);
  ...
  double eintri, emultii;
  calcprojectedHamiltonian(nfix, Qfix, &Q, &Int, j, par, ival, &angpara, &cmpara,
                           &eintri, &emultii);
  ...
  double einitial = emultii + alpha*eintri;

  fprintf(stderr, "\ninitial:\tE = %8.3f MeV, Emulti = %8.3f MeV, Eintr = %8.3f MeV\n\n", 
	  hbc*einitial, hbc*emultii, hbc*eintri);
  ...
  // minimize !
  MinimizeDONLP2multivapp(&Int, j, par, ival, 
  ...
  double eintrf, emultif;

  calcprojectedHamiltonian(nfix, Qfix, &Q, &Int, j, par, ival, &angpara, &cmpara,
                           &eintrf, &emultif);

  double e = emultif + alpha*eintrf;

  fprintf(stderr, "\nfinal:\tE = %8.3f MeV, Emulti = %8.3f, Eintr = %8.3f MeV\n\n", 
	  hbc*e, hbc*emultif, hbc*eintrf);
  ...
  e = emultif + alpha*eintrf;

  fprintf(stderr, "\nboosted:\tE = %8.3f MeV, Emulti = %8.3f, Eintr = %8.3f MeV\n\n", 
	  hbc*e, hbc*emultif, hbc*eintrf);
  ...
  // save minimization result in MIN directory
  {
    ...
    ensuredir("MIN");
    char outfile[1024];
    sprintf(outfile, "MIN/%s-minmultivapp.%d", parafile, (int) time(NULL));
    ...
    fprintf(outfp, "\n# minimized %s for %s in %s parameterization\n"
            "# projected on J^pi = %s\n"
            "# K-mixing: threshold: %g, minnorm: %g\n",
            cm ? "< Hintr >" : "< H >", q.name, P.name, 
            AngmomtoStr(j, par == +1 ? 0 : 1),
            threshkmix, minnormkmix);
    for (int ifix=0; ifix<nfix; ifix++)
      fprintf(outfp, "# fixed config: %s\n", fixmbfile[ifix]);
    fprintf(outfp, "# optimized eigenvalue #%d\n"
            "# using %s interaction\n", 
            ival+1, Int.name);
  
    fprintf(outfp, "\n# initial: e = %8.3f MeV, emulti = %8.3f MeV, eintr = %8.3f MeV\n", 
            hbc*einitial, hbc*emultii, hbc*eintri);
    fprintf(outfp, "# final:   e = %8.3f MeV, emulti = %8.3f MeV, eintr = %8.3f MeV\n", 
            hbc*e, hbc*emultif, hbc*eintrf);
