/**
  ...
  \file minenergyconorthogonalproj.c

  minimize energy of constrained and projected Slater determinant
  that is orthogonal to a given projected Slater determinant
...
int main(int argc, char *argv[])
{
  ...
  while((c = getopt(argc, argv, "j:K:p:Lol:m:s:T:L:S:J:R:D:Q:O:B:G:")) != -1)
    switch (c) {
    case 'j':
      j = atoi(optarg);
      break;
...
/*   } */
  ...
  char* fixmbfile = argv[optind+2];
  char* parafile = argv[optind+3];
  ...
  SlaterDet Q0;
  if (readSlaterDetfromFile(&Q0, fixmbfile)) {
  ...
  double n0, h0;
#ifdef MPI
  calcprojectedHamiltonianmpi(&Q0, &Int, 
			      j, k, par, &AngPar,
			      &h0, &n0); 
#else
  calcprojectedHamiltonian(&Q0, &Int, 
			   j, k, par, &AngPar,
			   &h0, &n0); 
#endif
...
#ifdef MPI
  calcorthprojectedHamiltonianmpi(&Q0, h0, n0, 
				  &Q, &Int, 
				  j, k, par, &AngPar, 
				  &eintr, &einitial);
#else
  calcorthprojectedHamiltonian(&Q0, h0, n0, 
			       &Q, &Int, 
			       j, k, par, &AngPar, 
			       &eintr, &einitial);
#endif
  ...
  // minimize !
  MinimizeDONLP2orthproj(&Int, j, k, par, &AngPar, &Q0, h0, n0, 
			 Const, nconst, &P, &q, maxsteps,
			 log, parafile); 
  ...
  fprintf(outfp, "\n# minimized %s for %s in %s parameterization\n"
	  "# orthogonal to state %s\n"
	  "# using %s interaction\n", 
	  cm ? "< Hintr >" : "< H >", q.name, P.name, fixmbfile, Int.name);

  fprintf(outfp, "\n# projected on J^pi = %s, K = %d\n", 
          AngmomtoStr(j, par == +1 ? 0 : 1), k);

  fprintf(outfp, "\n# projected energy: %8.3f\n", e*hbc);
