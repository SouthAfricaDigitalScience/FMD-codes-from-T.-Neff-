...
int main(int argc, char* argv[])
{
  ...
  char c;
  while ((c = getopt(argc, argv, "c:nl:ogC:Pqv:")) != -1)
    switch (c) {
    case 'g':
      debug=1;
      break;
  ...
  snprintf(datafile, 255, "%s.denspmn", slaterdetfile);;
  ...
  for (v=0; v<3; v++)
    if (view[v]) {
      ...
      // difference proton-neutron density
      for (j=0; j<npoints; j++) {
	for (i=0; i<npoints; i++)
	  fprintf(datafp, "%f  ", 
		    (dens[i+j*npoints+npoints*npoints]-dens[i+j*npoints+2*npoints*npoints])/(0.5*rho0)); 
	fprintf(datafp, "\n");
      }

      // difference neutron-proton density
      for (j=0; j<npoints; j++) {
	for (i=0; i<npoints; i++)
	  fprintf(datafp, "%f  ", 
		  (dens[i+j*npoints+2*npoints*npoints]-dens[i+j*npoints+npoints*npoints])/(0.5*rho0));
	fprintf(datafp, "\n");
      }
    }
  ...
  snprintf(scriptfile, 255, "%s.denspmn.script", slaterdetfile);
  ...
  for (v=0; v<3; v++) {
    if (view[v]) {
      ...
      fprintf(scriptfp, "densityplot, denspmn(*,*,%d), %d, %f, %f, %f, %f, $\n",
  ...
  char epsfile[255];
  snprintf(epsfile, 255, "%s.denspmn.eps", slaterdetfile);
