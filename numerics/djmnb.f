      DOUBLE PRECISION FUNCTION DJMNB(AJ,AM,AN,BETA)

C     modified from CERNlib
C     angular momentum argumens 2*j, 2*m, 2*n
...
C     Singapore 1988
      ...
      INTEGER JPM, JPN, JMN, JMM, MPN, SX, CX, K, K0, KQ, Q 
      DOUBLE PRECISION F, MPI, FCL, B, C, S, R, RT

      DIMENSION FCL(0:50)


      PARAMETER (F = 0.5D0)
      PARAMETER (MPI = 3.14159265358979323846D0)
      ...
      B=F*BETA
      S=LOG(MAX(1.0D-16,SIN(B)))
      C=LOG(MAX(1.0D-16,ABS(COS(B))))
      RT=F*(FCL(JPM)+FCL(JMM)+FCL(JPN)+FCL(JMN))
      K0=MAX(0,MPN)
      KQ=K0+JPM
      IF(BETA .GT. MPI) KQ=KQ+MPN
      Q=(-1)**KQ
      KQ=K0+K0
      CX=KQ-MPN
      SX=JPM+JPN-KQ
