/**
  ...
  Substract Tcm of intrinsic state to avoid center-of-mass excitations
...
static struct {
  ...
  gradSlaterDet* dhintr;
  gradSlaterDet* dtcmintr;
  gradSlaterDet* dnintr;
...
{       
  ...
  Work.dhintr = malloc(sizeof(gradSlaterDet));
  Work.dtcmintr = malloc(sizeof(gradSlaterDet));
  Work.dnintr = malloc(sizeof(gradSlaterDet));
  Work.dhproj = malloc(sizeof(gradSlaterDet));
  Work.dnproj = malloc(sizeof(gradSlaterDet));

  initgradSlaterDet(Q, Work.dhintr);
  initgradSlaterDet(Q, Work.dtcmintr);
  initgradSlaterDet(Q, Work.dnintr);
                              ...
                              double* eintr, double* eproj)
{
  ...
  complex double hintr, tcmintr, nintr;
  ...
  for (i=0; i<dim; i++)
    fprintf(stderr, "Q eigenstate %d: E = %8.3f MeV, n = %8.7f, J2 = %6.3f\n", i, 
            hbc*creal(hproj[i]/nproj[i]-tcmintr/nintr), creal(n2proj[i]/(nintr*nproj[i])), creal(j2proj[i]/nproj[i]));

  *eintr = (hintr-tcmintr)/nintr;
  *eproj = creal(hproj[ival]/nproj[ival]-tcmintr/nintr);
}



// gradients will be written to Work.dhintr, Work.dhproj
				  ...
				  double* eintr, double* eproj)
{
  ...
  gradSlaterDet* dhintr = Work.dhintr;
  gradSlaterDet* dtcmintr = Work.dtcmintr;
  gradSlaterDet* dnintr = Work.dnintr;
  ...
  *eintr= creal((hintr-tcmintr)/nintr);
  ...
  *eproj = creal(hproj[ival]/nproj[ival]-tcmintr/nintr);
  ...
  // gradient from intrinsic energy

  calcSlaterDetAux(Q, X);
  calcgradSlaterDetAux(Q, X, dX);
  calcgradHamiltonian(Int, Q, X, dX, dhintr);

  // trap: calcgradTCM calculates gradient of -Tcm !
  zerogradSlaterDet(dtcmintr);
  calcgradTCM(Q, X, dX, dtcmintr);

  addmulttogradSlaterDet(dhproj, dtcmintr, +1.0);
  addmulttogradSlaterDet(dhintr, dtcmintr, +1.0);
}				  
...
void FORTRAN(egradf)(const double* x, double* gradf)
{
  ...
  addmulttogradSlaterDet(Work.dhproj, Work.dhintr, Min.alpha);
